...
##Random forest AUC
test.predictions <- predict(rf.newdata, newdata=testdata)
test.predictions <- as.numeric(test.predictions)
roccurve_forest <- roc(unlist(testdata$itu_admission)~ test.predictions)
auc(roccurve_forest)
plot(roccurve_forest)
ci.auc(roccurve_forest)
...
# GRAPH THAT COMBINE RANDOM FOREST AND RIDGE AUC

plot.roc(y2[test2]~ ridge.coeff2,percentage=T,col="red",smooth=F)# for NEWS+qVIEWS
grid(NULL,NULL,col = "lightgray", lty = "dotted",lwd = par("lwd"), equilogs = TRUE)
lines.roc(y1[test1]~ ridge.coeff1,percentage=T,col="blue",smooth=F)# For NEWS only
lines.roc(unlist(testdata$itu_admission)~ test.predictions,percentage=F,col="orange",smooth=F)
legend("topleft", legend=c("Ridge1 56.2% [47-65]", "Ridge2 56.6% [47-65]","Forest 71.5% [65-77]"),
       col=c("red", "blue","orange"), lty=c(1,1,1), cex=0.8)
